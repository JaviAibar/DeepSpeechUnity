...
def main():
    ...
    if args.extended:
        ...
        if True:
            ...
            #jsonfile = open("metadatas.json", "w")

            for id, t in enumerate(metadata):

                #jsonfile.write(json.dumps(t))
            ...
            if args.output and args.output[-1] != "\\":
                args.output += "\\"
            if args.srt_name and args.srt_name[-4:] != ".srt":
                args.srt_name += ".srt"
            f = open((args.output if args.output else "") + (args.srt_name if args.srt_name else "output.srt"), "w")
            f.write(res)
        #    jsonfile.close()
            #print("Guardado? :O" + dirname(sys.argv[0]))
