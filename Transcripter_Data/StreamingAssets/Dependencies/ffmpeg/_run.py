...
os.chdir(str(Path(__file__).absolute().parents[1]))
...
@output_operator()
def compile(stream_spec, cmd=str(os.getcwd())+'/ffmpeg.exe', overwrite_output=False):
...
def run_async(
    stream_spec,
    cmd=str(os.getcwd()).replace("\\", "/")+'/ffmpeg.exe',
    pipe_stdin=False,
    pipe_stdout=False,
    pipe_stderr=False,
    quiet=False,
